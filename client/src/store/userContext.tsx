...
export const UserProvider: React.FC<any> = ({children}: {children: ReactNode}) => {
	...
	const registration = async (email: string, username: string, password: string, passwordConfirm: string) => {
        try {
            const response = await AuthService.registration(email, username, password, passwordConfirm);
	...
	return (
		<UserContext.Provider value={ {user, setUser, isAuth, setAuth, registration} } >
            {children}
